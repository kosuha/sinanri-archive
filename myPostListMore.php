...
//남은 post의 갯수
  $count = "SELECT COUNT(*) as cnt FROM generalForum WHERE memberId = '{$memberId}' AND postSeq<'{$lastSeq}'";
...
if($cnt['cnt'] < $showNum){ //남은 포스트가 %개 미만일 경우
  ...
  $sql = "SELECT * FROM generalForum WHERE memberId = '{$memberId}' AND postSeq<'{$lastSeq}' ORDER BY postSeq DESC";
  ...
  $sql = "SELECT * FROM generalForum WHERE memberId = '{$memberId}' AND postSeq<'{$lastSeq}' ORDER BY postSeq DESC LIMIT $showNum";
  $result = mysqli_query($conn, $sql);
  $list1 = array();
  $list2 = array();
  $list3 = array();
